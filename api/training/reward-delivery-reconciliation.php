...
try {
    if ($method === 'GET') {
        ...
        tl_security_json_response(['ok'=>true,'data'=>tl_stage893_summary()]);
        exit;
    }
    if ($method !== 'POST') {
    ...
    if ($action === 'stage893_reconcile_delivery') {
        $result = tl_stage893_reconcile_handoff($input);
    } elseif ($action === 'stage893_reconcile_delivery_batch') {
        $result = tl_stage893_reconcile_batch($input);
    } else {
        throw new TlHttpException('Unsupported Stage 893 action.', 422, 'unsupported_stage893_action');
    }
    tl_security_json_response(['ok'=>true,'action'=>$action,'data'=>$result,'summary'=>tl_stage893_summary()]);
} catch (Throwable $e) {
