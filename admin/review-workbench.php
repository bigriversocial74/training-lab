...
?>
<style>
  .labs-review-workbench-stack,
  .labs-review-workbench-stack .labs-panel-list,
  .labs-review-workbench-stack .labs-stage35-review-list{
    display:grid;
    grid-template-columns:1fr !important;
    gap:16px;
  }
  .labs-review-workbench-stack .labs-flow-grid,
  .labs-review-workbench-stack .labs-kpis,
  .labs-review-workbench-stack .labs-admin-band{
    display:grid;
    grid-template-columns:1fr !important;
    gap:16px;
  }
  .labs-review-workbench-stack .labs-panel-item,
  .labs-review-workbench-stack .labs-stage885-proof-card{
    grid-template-columns:44px 1fr !important;
  }
  .labs-review-workbench-stack .labs-page-title{
    align-items:flex-start;
    flex-direction:column;
  }
</style>
<div class="labs-review-workbench-stack">
...
<section class="labs-page-title labs-stage200-title"><div><span class="labs-eyebrow">Review workbench</span><h1>Decide proof and move the reward lifecycle forward.</h1><p class="labs-copy">Approvals create Training Lab receipts and eligible reward previews only. Microgifter issuing, claims, wallets, and payments remain closed.</p></div><div class="labs-actions"><a class="labs-btn" href="<?php echo labs_url('/api/training/proof-review-workflow.php'); ?>">Stage 885 API</a><a class="labs-btn" href="<?php echo labs_url('/admin/reward-bridge.php'); ?>">Reward Bridge</a><a class="labs-btn labs-btn-primary" href="<?php echo labs_url('/app/flow-board.php'); ?>">Flow Board</a></div></section>
<section class="labs-card"><h2>Review totals</h2><div class="labs-panel-list"><div class="labs-panel-item"><span class="labs-mini-icon">P</span><div><strong><?php echo count($pending); ?> pending</strong><p>Proof decisions</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">R</span><div><strong><?php echo count($recent); ?> recent</strong><p>Latest review decisions</p></div></div></div></section>
<section class="labs-card"><h2>Legacy decision queue</h2><div class="labs-stage35-review-list"><?php foreach ($pending as $proof): ?><article class="labs-stage35-review-item"><div><span class="labs-pill"><?php echo labs_e((string)$proof['status']); ?></span><h3><?php echo labs_e((string)($proof['task_title'] ?? 'Training proof')); ?></h3><p><?php echo labs_e((string)($proof['campaign_title'] ?? 'Campaign')); ?> · <?php echo labs_e((string)($proof['participant_label'] ?? 'Participant')); ?></p><blockquote><?php echo labs_e((string)($proof['proof_text'] ?? 'No proof text.')); ?></blockquote></div><form action="<?php echo labs_url('/admin/action-result.php'); ?>" method="post" class="labs-stage30-form labs-stage35-decision-form"><input type="hidden" name="confirm_training_action" value="1"><input type="hidden" name="training_action" value="review_proof"><input type="hidden" name="proof_id" value="<?php echo (int)$proof['id']; ?>"><label>Decision<select name="decision"><option value="approved">Approve</option><option value="needs_more_info">Needs more info</option><option value="rejected">Reject</option></select></label><label>Review notes<textarea name="review_notes" rows="3">Reviewed in Stage 200 Review Workbench.</textarea></label><button class="labs-btn labs-btn-primary" type="submit">Submit Decision</button></form></article><?php endforeach; ?><?php if (!$pending): ?><div class="labs-empty-state"><strong>No pending proof</strong><p>Submitted proof will appear here. Complete a proof-required task to test the review loop.</p></div><?php endif; ?></div></section>
...
<section class="labs-card labs-stage240-panel">
  ...
  <div class="labs-panel-list"><div class="labs-panel-item"><span class="labs-mini-icon">P</span><div><strong><?php echo (int)$stage240Sla['counts']['pending']; ?> pending</strong><p>Submitted/in review</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">24</span><div><strong><?php echo (int)$stage240Sla['counts']['over_24h']; ?> over 24h</strong><p>SLA watch</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">S</span><div><strong><?php echo (int)$stage240Sla['score']; ?>/100</strong><p>Review health score</p></div></div></div>
...
<section class="labs-card">
  ...
  <div class="labs-panel-list"><div class="labs-panel-item"><span class="labs-mini-icon">S</span><div><strong><?php echo (int)$stage280Review['score']; ?>/100</strong><p>Review loop</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">T</span><div><strong><?php echo (int)$stage280Review['counts']['total']; ?> total</strong><p>Decisions</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">P</span><div><strong><?php echo (int)$stage280Review['counts']['pending_proofs']; ?> pending</strong><p>Proof queue</p></div></div></div>
