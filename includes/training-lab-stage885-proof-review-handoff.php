...
if (!function_exists('tl_stage885_render_workflow')) {
    function tl_stage885_render_workflow(?string $proofRef = null): void
    {
        ...
        echo '<section class="labs-page-title labs-stage885-single-title"><div><span class="labs-eyebrow">Stage 885</span><h1>Proof Review + Award Handoff Preview</h1><p class="labs-copy">Review submitted proof, record a Training Lab decision, and generate a preview-only Microgifter award handoff. Production issuing remains closed.</p></div><div class="labs-actions"><a class="labs-btn labs-btn-primary" href="' . tl_stage885_e(function_exists('labs_url') ? labs_url('/api/training/proof-review-workflow.php') : '/api/training/proof-review-workflow.php') . '">View JSON</a><a class="labs-btn" href="' . tl_stage885_e(function_exists('labs_url') ? labs_url('/admin/reward-bridge.php') : '/admin/reward-bridge.php') . '">Reward Bridge</a></div></section>';
        echo '<section class="labs-card labs-stage885-single-card"><h2>Workflow status</h2><div class="labs-panel-list"><div class="labs-panel-item"><span class="labs-mini-icon">A</span><div><strong>' . (!empty($summary['accepted']) ? 'Accepted' : 'Check') . '</strong><p>Stage 885 workflow gate</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">Q</span><div><strong>' . (int)$summary['queue_count'] . ' proof row(s)</strong><p>Total review queue</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">P</span><div><strong>' . (int)$summary['pending_review_count'] . ' pending</strong><p>Submitted or in review</p></div></div><div class="labs-panel-item"><span class="labs-mini-icon">H</span><div><strong>' . tl_stage885_e((string)($handoff['status'] ?? 'preview')) . '</strong><p>Award handoff remains preview-only</p></div></div></div></section>';
        echo '<section class="labs-card labs-stage885-single-card"><h2>Decision queue</h2><div class="labs-panel-list">';
        foreach ($queue as $row) {
            $publicId = (string)($row['public_id'] ?? '');
            echo '<article class="labs-panel-item labs-stage885-proof-card"><span class="labs-mini-icon">R</span><div><span class="labs-pill">' . tl_stage885_e((string)($row['proof_status'] ?? 'submitted')) . '</span><h3>' . tl_stage885_e((string)($row['task_title'] ?? 'Training task')) . '</h3><p>' . tl_stage885_e((string)($row['campaign_title'] ?? 'Campaign')) . ' · ' . tl_stage885_e((string)($row['participant_label'] ?? 'Participant')) . '</p><p><strong>Latest review:</strong> ' . tl_stage885_e((string)($row['latest_decision'] ?? 'pending')) . '</p><form action="' . tl_stage885_e(function_exists('labs_url') ? labs_url('/admin/action-result.php') : '/admin/action-result.php') . '" method="post" class="labs-stage30-form"><input type="hidden" name="confirm_training_action" value="1"><input type="hidden" name="training_action" value="stage885_review_proof"><input type="hidden" name="proof_id" value="' . tl_stage885_e($publicId) . '"><label>Decision<select name="decision"><option value="approved">Approve</option><option value="needs_more_info">Needs more info</option><option value="rejected">Reject</option></select></label><label>Notes<textarea name="review_notes" rows="2">Stage 885 controlled proof review.</textarea></label><button class="labs-btn labs-btn-primary" type="submit">Submit</button></form></div></article>';
        }
        if (!$queue) echo '<div class="labs-empty-state"><strong>No proof rows found yet.</strong><p>Submitted proof will appear here.</p></div>';
        echo '</div></section>';
        echo '<section class="labs-card labs-stage885-single-card"><h2>Award handoff preview</h2><pre class="labs-stage25-code">' . tl_stage885_e(json_encode($handoff, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)) . '</pre></section>';
        echo '<section class="labs-safe-note">Stage 885 is single-column. It writes Training Lab review/receipt/eligibility records only. It does not issue Microgifter rewards, mutate wallets, process payments, redeem claims, or destructively sync.</section>';
    }
}
