...
if (!function_exists('tl_stage883_safe_call')) {
    ...
    {
        if (!function_exists($fn)) return ['called' => false, 'ok' => false, 'result' => null, 'error' => 'function missing'];
        try {
            $result = $fn(...$args);
...
if (!function_exists('tl_stage883_first_read')) {
    function tl_stage883_first_read(array $functions, array $args = [], bool $requiresKey = true): array
    {
        ...
        if (!$available) return ['source' => 'fixture', 'function' => '', 'available_functions' => [], 'connected' => false, 'developer_key_present' => $hasKey, 'rows' => [], 'error' => 'no adapter function found'];
        if ($requiresKey && !$hasKey) return ['source' => 'missing_key', 'function' => $available[0], 'available_functions' => $available, 'connected' => false, 'developer_key_present' => false, 'rows' => [], 'error' => 'developer key missing'];
...
if (!function_exists('tl_stage883_read_health_rows')) {
    ...
    {
        $contracts = [
            'merchant_campaign_catalog' => ['label'=>'Merchant campaign catalog', 'read'=>tl_stage883_first_read(['microgifter_training_campaign_catalog','microgifter_merchant_reward_campaigns','microgifter_reward_campaign_catalog','microgifter_reward_catalog']), 'fallback'=>function_exists('tl_stage800_imported_campaigns') ? tl_stage800_imported_campaigns() : [], 'required_any'=>['campaign_id','id','campaign_name','name','title']],
            'customer_awards' => ['label'=>'Customer awards', 'read'=>tl_stage883_first_read(['microgifter_training_user_awards','microgifter_customer_awards','microgifter_user_awards'], [$userId]), 'fallback'=>function_exists('tl_stage840_user_awards') ? tl_stage840_user_awards($userId) : [], 'required_any'=>['award_id','id','award_title','title']],
            'customer_account_status' => ['label'=>'Customer account status', 'read'=>tl_stage883_first_read(['microgifter_user_account_status','microgifter_customer_account_status','microgifter_training_user_account_status'], [$userId]), 'fallback'=>function_exists('tl_stage840_customer_account_bridge') ? [tl_stage840_customer_account_bridge($userId)] : [], 'required_any'=>['status','connected','customer_account','display_name','email']],
            'adapter_status' => ['label'=>'Adapter status', 'read'=>tl_stage883_first_read(['microgifter_adapter_status','microgifter_training_sync_status','microgifter_adapter_sync_status'], [], false), 'fallback'=>function_exists('tl_stage880_adapter_mode') ? [tl_stage880_adapter_mode()] : [], 'required_any'=>['status','mode','connected','developer_key_present']],
            'inventory_freshness' => ['label'=>'Inventory freshness', 'read'=>tl_stage883_first_read(['microgifter_campaign_sync_health','microgifter_reward_inventory_refresh_preview','microgifter_adapter_sync_status','microgifter_training_sync_status'], [], false), 'fallback'=>function_exists('tl_stage880_campaign_sync_health') ? [tl_stage880_campaign_sync_health()] : [], 'required_any'=>['inventory_freshness','freshness_status','status','mode']],
        ];
        $rows = [];
        foreach ($contracts as $key => $contract) {
            ...
            $shapeScore = tl_stage883_shape_score($dataRows, (array)$contract['required_any']);
...
if (!function_exists('tl_stage883_readonly_summary')) {
    function tl_stage883_readonly_summary(int $userId = 0): array
    {
        ...
        $adapterCount = 0; $fixtureCount = 0; $missingKeyCount = 0;
        foreach ($rows as $row) {
        ...
        return [
            ...
            'next_recommended_step' => $adapterCount > 0 ? 'Review real read adapter payloads and then plan controlled handoff approval without enabling production mutation.' : 'Configure real read adapter functions and developer key when ready; fixture fallback remains safe.',
        ];
    }
}
...
if (!function_exists('tl_stage883_render_readonly_adapter')) {
    function tl_stage883_render_readonly_adapter(int $userId = 0): void
    {
        ...
        echo '</section><section class="labs-card"><h2>Read contract health</h2>';
