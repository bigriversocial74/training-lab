...
if (!function_exists('tl_public_img')) {
    function tl_public_img(string $key, string $class = '', string $alt = ''): string
    {
        ...
        return '<img class="' . tl_public_e($class) . '" src="' . tl_public_e($url) . '" alt="' . tl_public_e($label) . '" loading="lazy">';
    }
}
if (!function_exists('tl_public_site_header')) {
    ...
    {
        ?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title><?php echo tl_public_e($title); ?></title><meta name="description" content="<?php echo tl_public_e($description); ?>"><link rel="stylesheet" href="<?php echo tl_public_e(labs_asset('css/public-template.css')); ?>"></head><body><div class="tl-shell tl-template-shell"><header class="tl-container tl-header tl-template-header"><a class="tl-brand tl-template-brand" href="<?php echo tl_public_e(labs_url('/')); ?>" aria-label="Training Lab home"><span class="tl-logo-flask"><?php echo tl_public_img('flask', '', 'Training Lab'); ?></span><span class="tl-brand-text"><span class="tl-brand-title">Training Lab</span><span class="tl-brand-sub">by Microgifter</span></span></a><button class="tl-menu-toggle" type="button" aria-label="Open menu" aria-controls="tl-primary-nav" aria-expanded="false" data-tl-menu-open><span></span><span></span><span></span></button><nav class="tl-nav tl-template-nav" id="tl-primary-nav" aria-label="Primary navigation"><button class="tl-nav-close" type="button" aria-label="Close menu" data-tl-menu-close>&times;</button><a class="<?php echo $active === 'product' ? 'is-active' : ''; ?>" href="<?php echo tl_public_e(labs_url('/how-it-works.php')); ?>">Product <span aria-hidden="true">⌄</span></a><a class="<?php echo $active === 'pricing' ? 'is-active' : ''; ?>" href="<?php echo tl_public_e(labs_url('/pricing.php')); ?>">Pricing</a><a class="<?php echo $active === 'about' ? 'is-active' : ''; ?>" href="<?php echo tl_public_e(labs_url('/about.php')); ?>">About</a><a class="<?php echo $active === 'blog' ? 'is-active' : ''; ?>" href="<?php echo tl_public_e(labs_url('/blog.php')); ?>">Blog</a><a class="tl-btn tl-btn-outline" href="<?php echo tl_public_e(labs_url($authHref)); ?>"><?php echo tl_public_e($authLabel); ?></a></nav><div class="tl-nav-overlay" data-tl-menu-close></div></header><?php
    }
}
...
if (!function_exists('tl_public_auth_status')) {
    ...
    {
        if ($error) echo '<div class="tl-form-alert tl-form-error"><strong>Account issue</strong><p>' . tl_public_e($error) . '</p></div>';
        if ($result) echo '<div class="tl-form-alert tl-form-success"><strong>Account action complete</strong><p>' . tl_public_e((string)($result['message'] ?? $result['sync_status'] ?? 'Your account session was updated.')) . '</p></div>';
    }
}
