...
$check(str_contains($service, "h.failure_code='external_delivery_confirmation_required'"), 'candidate query includes quarantined handoffs');
$check(str_contains($service, "h.handoff_status='delivered' AND re.status NOT IN ('issued','linked')"), 'candidate query includes local delivery mismatches');
